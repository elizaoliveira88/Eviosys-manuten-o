...
export class DisplayDayPipe implements PipeTransform {
    ...
    transform (value: any, offset: any = 'DEF'): string {
        if(value == null || value == '') { return ''; }
        var time = this.getNumberValue(value);
        if(isNaN(time)) { return ''; }
        return this.datePipe.transform(new Date(time), this.masterDataStore.timeFormat.Datfm, offset === 'DEF' ? undefined : offset);
    }
